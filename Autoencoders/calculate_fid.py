...
def get_activations(images, clf, device):
    all_activations = []
    for img in images:
        img = img.to(device)
        activations = clf.get_activations(img)
        activations = activations.detach().cpu().numpy().reshape(activations.size(0), -1)
...
def calculate_activation_statistics(dataloader, model, classifier):
    ...
    for batch in dataloader:
        for img in batch[0]:
            images_real.append(img.squeeze())
            images_ae.append(model(img.unsqueeze(0).to(device)).squeeze().detach().to('cpu'))
    act = get_activations(images_real, classifier, device)

    mu1 = np.mean(act, axis=0)
    sigma1 = np.cov(act, rowvar=False)

    act = get_activations(images_ae, classifier, device)
