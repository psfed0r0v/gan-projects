...
def calculate_activation_statistics(dataloader, model, inception, attrs_len):
    inception.eval()
    model.eval()
    real_activations, fake_activations = [], []

    for batch in tqdm(dataloader, desc='fid loop', leave=False):
        x, label = batch[0].to('cuda'), batch[1].to('cuda')
        ...
        label = label.view(-1, attrs_len, 1, 1)

        fake = inception(model(x, label))[0]
...
@torch.no_grad()
def calculate_fid(model, dataloader, attrs_len=5, dims=2048):
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    inception = InceptionV3([block_idx], normalize_input=False).to('cuda')
    model.to('cuda')

    m1, s1, m2, s2 = calculate_activation_statistics(dataloader, model, inception, attrs_len=attrs_len)
