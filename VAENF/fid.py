...
def get_activations(real_data, inception, batch_size=16):
    inception.eval()
    activations = []

    for i in range(0, len(real_data), batch_size):

        batch = torch.from_numpy(real_data[i: i + batch_size]).float().to('cuda')
...
def calculate_activation_statistics(dataloader, model, inception, device, img_size):
    inception.eval()
    model.eval()
    real_activations, fake_activations = [], []

    for data, label in tqdm(dataloader, desc='fid loop', leave=False):
        x = data.to(device)
        real = inception(x)[0]
        ...
        fake = inception(model.sample(dataloader.batch_size, img_size))[0]
...
@torch.no_grad()
def calculate_fid(model, dataloader, device, img_size=64, normalize_input=False, dims=2048):
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    inception = InceptionV3([block_idx], normalize_input=normalize_input).to(device)
    model.to(device)

    m1, s1, m2, s2 = calculate_activation_statistics(dataloader, model, inception, device, img_size)
